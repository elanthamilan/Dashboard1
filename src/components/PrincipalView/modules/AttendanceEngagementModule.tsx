...
// - All parts of this step seem to be covered in the generated code.The `AttendanceEngagementModule.tsx` file has been updated with the new "At-Risk Students (Attendance)" list and detail view functionalities.

This involved:
- Adding state variables: `viewingAtRiskStudents` and `selectedStudentForAttendanceDetail`.
- Adding a button on the main overview to switch to the at-risk view.
- Updating breadcrumbs to reflect the current view (overview, program drilldown, semester drilldown, class drilldown, at-risk list, or student detail).
- Implementing conditional rendering to display the correct view based on the component's state.
- Creating `atRiskStudentsData` using `useMemo` to:
    - Collect all students from `institutionData` respecting global filters (academic year, degree, department, program).
    - For each student, calculate their attendance percentage and count of absences over the last `AT_RISK_PERIOD_DAYS` (30 days).
    - Filter this list to include only students with attendance below `AT_RISK_THRESHOLD` (75%).
    - Sort the at-risk students by their attendance rate.
- Displaying the at-risk students in an Ant Design `Table` with columns: Student Name, Student ID, Program Name, Calculated Attendance (%), and Recent Absences. Rows are clickable (or have a "View Details" button) to set `selectedStudentForAttendanceDetail`.
- Creating a detailed view for a selected student, which shows:
    - A title with the student's name.
    - Statistics for their calculated attendance rate and recent absences over the defined period.
    - An Ant Design `Timeline` displaying their attendance records for that period (date, status, course, reason, notes).
- Ensuring "Back" buttons and breadcrumb links allow for correct navigation between these views.

The next step is to add all the new translation keys.

**New Translation Keys to add:**
*   `module.attendance.viewAtRiskButton: "View At-Risk Students (Attendance)"`
*   `module.attendance.atRiskStudentsTitle: "At-Risk Students (Low Attendance)"`
*   `module.attendance.studentAttendanceDetailTitle: "Attendance Details for {studentName}"`
*   `module.attendance.backToAtRiskList: "Back to At-Risk List"`
*   Table headers for at-risk list (some might be reused if identical to existing program/student table headers):
    *   `module.attendance.studentName: "Student Name"` (likely exists)
    *   `module.attendance.studentId: "Student ID"` (likely exists)
    *   `module.attendance.programName: "Program"` (likely exists)
    *   `module.attendance.calculatedAttendanceRate: "Attendance Rate (%)"`
    *   `module.attendance.recentAbsences: "Recent Absences (Last 30d)"`
*   Timeline/log details:
    *   `attendanceStatus.present`: "Present" (exists)
    *   `attendanceStatus.absent`: "Absent" (exists)
    *   `attendanceStatus.late`: "Late"
    *   `attendanceStatus.excused`: "Excused"
    *   `module.attendance.attendanceLogTitle`: "Attendance Log (Last 30 Days)"
*   `common.noRecentAttendanceData`: "No recent attendance records for this student."
*   `common.noAtRiskStudents`: "No students currently identified as at-risk based on attendance."
