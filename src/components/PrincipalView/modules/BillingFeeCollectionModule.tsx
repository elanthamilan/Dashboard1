...
// All seems in order for this step.The `BillingFeeCollectionModule.tsx` has been updated to incorporate the functionality for selecting a semester within a department, viewing its invoices, and then displaying details of a selected invoice in a modal.

Key changes made:
1.  **State Variables**:
    *   `selectedSemesterForInvoices: (SemesterBillingInfo & { departmentName?: string }) | null` was added to store the semester selected from the department view. `SemesterBillingInfo` is an augmented type that includes billing aggregates and contextual names.
    *   `selectedInvoiceForDetail: Invoice | null` to hold the invoice chosen for detailed view.
    *   `isInvoiceDetailModalVisible: boolean` to control the visibility of the invoice detail modal.

2.  **Semester Table Interactivity**:
    *   The table displaying semesters within a selected department now has a "View Invoices" button on each row.
    *   Clicking this button calls `handleSemesterSelect`, passing the selected semester data (which includes `programName` and `departmentName` for context), and updates the `selectedSemesterForInvoices` state.

3.  **Breadcrumb Updates**:
    *   Breadcrumbs now dynamically adjust to show the path: "Billing & Fee Collection" / "{Department Name}" / "{Semester Name}".
    *   Links in the breadcrumbs allow navigation back up the hierarchy (e.g., clicking the department name goes back to the semester list for that department).

4.  **Conditional Rendering for Invoice List**:
    *   If `selectedDepartmentForBilling` is set AND `selectedSemesterForInvoices` is also set, the module now renders a table listing invoices for that specific semester.
    *   The department semester view and the main overview are rendered if the respective selections are not made.

5.  **Data Preparation for Invoice List (`invoicesInSelectedSemester`)**:
    *   A `useMemo` hook was created to prepare the list of invoices.
    *   It filters `allInvoices` based on:
        *   Student IDs present in `selectedSemesterForInvoices.students`.
        *   Ensuring `invoice.issueDate` falls within the selected semester's `startDate` and `endDate`.
        *   Additionally respects the global `filters.dateRange` if applied.
    *   Each invoice in the list is augmented with `studentName` by looking up the `studentId` in the `allStudentsSummaryList` (which is populated in `useEffect` and includes `programName`).
    *   The list is sorted by issue date.

6.  **UI for Invoice List Table**:
    *   An Ant Design `Table` displays the filtered invoices.
    *   Columns include: Invoice ID, Student Name, Total Amount, Due Date, and Status (with a color-coded `Tag`).
    *   A "View Details" button (`FileTextOutlined` icon) in each row sets `selectedInvoiceForDetail` and toggles `isInvoiceDetailModalVisible` to true.

7.  **UI for Invoice Detail Modal**:
    *   An Ant Design `Modal` is used, controlled by `isInvoiceDetailModalVisible`.
    *   It displays comprehensive details of the `selectedInvoiceForDetail`:
        *   Basic info: Invoice ID, Student Name, Total Amount, Due Date, Status (in a `Descriptions` component).
        *   Line Items: A nested `Table` lists each fee item (`description`, `amount`) within the invoice.
        *   Payment History: Another nested `Table` lists payments made towards this invoice (Payment Date, Amount Paid, Method), derived by filtering `allPayments`.
    *   A "Close" button in the modal footer hides it.

8.  **Navigation**:
    *   "Back to Semesters in {departmentName}" button is displayed in the invoice list view.
    *   The department `Select` dropdown is disabled when viewing invoices of a semester to guide navigation via buttons and breadcrumbs.

The next step is to add the new translations to `en.json` and `es.fixed.json`.

**New Translation Keys to add:**
*   `module.billing.viewInvoicesButton`: "View Invoices"
*   `module.billing.invoiceListTitle`: "Invoices for {programName} - {semesterName}"
*   `module.billing.backToSemesterList`: "Back to Semesters for {departmentName}" (Note: existing `backToSemesterList` key in Attendance module is slightly different, so this should be specific to billing or a more generic one created if applicable)
*   Invoice table headers:
    *   `module.billing.invoiceId`: "Invoice ID"
    *   `module.billing.studentName`: "Student Name" (likely exists)
    *   `module.billing.amount`: "Amount" (can be reused)
    *   `module.billing.dueDate`: "Due Date"
    *   `module.billing.status`: "Status" (likely exists)
*   `module.billing.invoiceDetailTitle`: "Invoice Details - {invoiceId}"
*   Detail labels for modal:
    *   `module.billing.lineItems`: "Line Items"
    *   `module.billing.paymentHistory`: "Payment History"
    *   `module.billing.paymentDate`: "Payment Date"
    *   `module.billing.amountPaid`: "Amount Paid"
    *   `module.billing.method`: "Method" (can be reused)
*   `common.viewDetails` (reused for button text)
*   `common.noInvoicesFound`: "No invoices found for this semester."
*   `common.noPaymentsMade`: "No payments made for this invoice."

I'll start by updating `en.json`.
